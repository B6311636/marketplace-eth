...
import { Button, Model } from "@components/ui/common";
...
const createFormState = ({ price, email, confirmationEmail }, hasAgreedTOS) => {
    if (!price || Number(price) <= 0) {
        ...
        return _createFormState(true, "You need to agree with teams of service in order to submit the from")
    }

    return _createFormState()
}

export default function OrderModel({ course, onClose, onSubmit }) {
    ...
    const fromState = createFormState(order, hasAgreedTOS)
